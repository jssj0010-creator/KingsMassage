...
(function () {
  ...
  function log(msg, obj){
    try{ console.log("[RegionMap]", msg, obj||""); }catch(e){}
    var el = document.querySelector('#region-map .map-debug');
    if(!el){ el = document.createElement('div'); el.className='map-debug'; document.getElementById('region-map').appendChild(el); }
    el.textContent = (typeof msg==='string'?msg:JSON.stringify(msg)) + (obj?(" "+JSON.stringify(obj)):"");
  }
  ...
  function init(lat,lng,label){
    ...
    // Always show static image first
    var img=new Image(); img.src=staticMapUrl(lat,lng,12); img.alt=(label||"지역")+" 위치 지도"; img.className="map-fallback";
    holder.appendChild(img);

    // Try to load Leaflet (if CSP blocks, static image remains)
    function loadLeaflet(){ if(window.L) return render();
      var lc=document.createElement('link');lc.rel='stylesheet';lc.href='https://unpkg.com/leaflet@1.9.4/dist/leaflet.css';document.head.appendChild(lc);
      var s=document.createElement('script');s.src='https://unpkg.com/leaflet@1.9.4/dist/leaflet.js';s.onload=render; s.onerror=function(){ log("Leaflet load failed"); }; document.head.appendChild(s);}
    var rendered=false; function render(){ if(rendered) return; rendered=true; try{
      ...
      log("Leaflet ready");
    }catch(e){ log("Leaflet error", e.message); } }
  ...
  function boot(){
    ...
    log("path="+location.pathname+" key="+key);
    var p = REGION_COORDS[key];
    if(!p){ log("coords not found for key; using fallback"); p = [37.5172,127.0473]; } // fallback 강남
